...
% Thus, the returned value is of the same shape as the model parameters, which by the way are not provided to this function. Notice that we're talking about 
%the mean over data cases (as opposed to the sum over data cases).
    m = size(visible_state,2);
    myPos = visible_state * hidden_state';
    
    %myNeg = 
    d_G_by_rbm_w= myPos'./ m;
end
