function a3(wd_coefficient, n_hid, n_iters, learning_rate, momentum_multiplier, do_early_stopping, mini_batch_size)
  ...
  if n_iters ~= 0,
    ...
    %title('blue is training data loss, red is validation loss');
...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
 ...
 % It contains a one-of-N encoding of the class, i.e. one element in every column is 1 and the others are 0.
  ...
  % The returned object is supposed to be exactly like parameter <model>, i.e. it has fields ret.input_to_hid and ret.hid_to_class. However, the contents 
  %of those matrices are gradients (d loss by d model parameter), instead of model parameters.
	 
  % This is the only function that you're expected to change. Right now, 
  %it just returns a lot of zeros, which is obviously not the correct output. Your job is to replace that by a correct computation.
  m = size(data.targets,2);
  %a1 = [ones(m,1) data.inputs];
  a1 = data.inputs;
  ...
  %weight the inputs
  z2 = theta1*a1;
  %Compute the activation (sigmoid) of the hidden layer
  a2 = sigmoid(z2);%1 ./ (1+exp(-z2));
  %add biases
  %a2 = [ones(m,1) a2];
 %weight the hidden layer outputs; 
  z3 = theta2*a2;
  %compute the activation function
  if (0)
    for cm = 1:m
      h3(:,cm) = logistic(z3(:,cm))/sum(logistic(z3(:,cm)));
    end%for cm 
  end
  %h3 = zeros(size(theta2,1),size(data.targets,1));
  for cm = 1:m
    h3(:,cm) = exp(z3(:,cm))/sum(exp(z3(:,cm)));
  end%for cm 
...
d3 = zeros(size(h3,1),m);
%implement as a loop first
if(0)
  for ck = 1:K
    d3(:,ck) = (h3(:,ck) - y(:,ck) );
  end%for ck
...
%d2 = a2' * d3;
   ...
   %.* hidden_layer_state .* (1 - hidden_layer_state);
      %bpd1 = (Theta2' * d3')' .* a2 .* (1-a2);

    %% HIDDEN LAYER.
    
d2 = (Theta2' *d3) .* (logistic(z2) .*(1-logistic(z2)));%sigmoidGradient(z2);

Delta1 = 0 *  Theta1;
Delta2 = 0 * Theta2;

for cm = 1:m
  Delta2 = Delta2 + d3(:,cm)*a2(:,cm)';

  Delta1 = Delta1 + d2(:,cm)*a1(:,cm)';
end%for cm

wd_loss1 =  wd_coefficient * (Theta1); 
wd_loss2 = wd_coefficient * (Theta2);


Theta1_grad = Delta1/(m) + wd_loss1;
Theta2_grad = Delta2/(m) + wd_loss2;
  whos
  ret.input_to_hid =  Theta1_grad; %model.input_to_hid';% * 0;
  ret.hid_to_class = Theta2_grad; % ret.input_to_hid * model.hid_to_class * 0;
end
