""" cNN based on tutorial from https://www.tensorflow.org/tutorials/layers - modified to classify 0 vs all other digits"""
...
def cNNMTModel(data, labels, mode):
    ...
    logits = tf.layers.dense(inputs=dropout,
                             units=2) # 2 units for 2 classes: w & w/o MTs
...
def main(unused_argv):
    ...
    for c in range(np.shape(trainLabels)[0]):
        if(trainLabels[c] < 1):
            trainLabels[c] = 0
        elif(trainLabels[c] >= 1):
            trainLabels[c] = 1
    for c in range(np.shape(evalLabels)[0]):
        if(evalLabels[c] < 1):
            evalLabels[c] = 0
        elif(evalLabels[c] >= 1):
            evalLabels[c] = 1
    ...
    MTClassifier = learn.Estimator(model_fn = cNNMTModel,
                                   model_dir = "./MNIST2Cat/MTConvNetModel")
    # set up logging
